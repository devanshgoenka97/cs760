...
class NeuralNetwork():
    ...
    def __init__(self, input_size = 2, output_size = 1, epochs = 100, batch_size = 20, learning_rate = 0.001):
        ...
        # Numerically more stable than BCELoss()
        self.criterion = nn.BCEWithLogitsLoss()
        # Build a simple 2-layer feed forward network as described
        self.model = nn.Sequential(nn.Linear(self.input_size, self.hidden_size, bias=True),
                      nn.Sigmoid(),
    ...
    def fit(self, X, y):
        """
        Trains the neural classifier with the given input and target labels.
