...
class DecisionTree():
    ...
    def _choose_attr(self, df):
        ...
        # Check across attributes to find maximum IG
        for attribute in self.attributes:
            thres = self._select_threshold(df, attribute)
            ig = self._info_gain(df, attribute, thres)
            print(str(attribute) + " Attribute, Threshold : " + str(thres) + " IG: " + str(ig))
            if ig > max_info_gain:
                max_info_gain = ig
                best_attr = attribute
                threshold = thres
    ...
    def _info_gain(self, df, attribute, threshold):
        ...
        remainder = self._remainder(df, [sub1, sub2])

        ig = self._info_entropy(df) - remainder
        return ig
